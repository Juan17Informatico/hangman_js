...
export const getWordsByDifficulty = (difficulty) => {
  ...
  return WORDS.filter((w) => w.difficulty >= min && w.difficulty <= max).map((entry) => ({
    ...entry,
    word: normalizeWord(entry.word)
  }));
};

export const buildWordSequence = (difficulty, wordsPerLevel) => {
  const pool = getWordsByDifficulty(difficulty);
  const shuffled = [...pool].sort(() => Math.random() - 0.5);
  const needed = Math.max(1, wordsPerLevel);
  const sequence = [];
